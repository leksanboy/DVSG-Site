<?php require_once('../../../Connections/conexion.php');
	class getDuration {
        ...
        public static function formatTime($timeInSeconds) //as hh:mm:ss
        {
            $timeInSeconds = round($timeInSeconds);
		    $timeInSeconds = abs($timeInSeconds);
		    $hours = floor($timeInSeconds / 3600) . ':';
		    if ($hours == '0:') $hours = '';
		    $minutes = substr('00' . floor(($timeInSeconds / 60) % 60), -2) . ':';
	...
	$FILE_NAME = 		$_SESSION['MM_Id'].'_'.time().'_'.rand().'.mp3'; // File name
	...
	if(move_uploaded_file($fileTmpLoc, "songs/$FILE_NAME")){
