<?php require_once('../../Connections/conexion.php');
    ...
    $tiempocotejo= time();
    $insertSQL = sprintf("INSERT INTO z_posts_comments (user, time, comment, page) VALUES (%s, %s, %s, %s)",
    ...
    actualiazarPost('addComment', $_POST['page']);

    mysql_select_db($database_conexion, $conexion);
    $query_GetNewComment = sprintf("SELECT * FROM z_posts_comments WHERE time=%s",$tiempocotejo,"int");
...
<div class="box" id="comentariosPostId<?php echo $row_GetNewComment['id'] ?>">
    ...
    <?php if (isset ($_SESSION['MM_Id'])){ ?> 
        <?php  if (($row_GetNewComment['user'] == $_SESSION['MM_Id']) || (rango_admin ($_SESSION['MM_Id']) ==4)) {?>
            <div class="delete" onClick="deleteComment('<?php echo $row_GetNewComment['id'] ?>','<?php echo $_POST['page'] ?>');">
