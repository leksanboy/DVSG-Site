...
if (!function_exists("GetSQLValueString")) {
	...
	function comprobacion_like_user($iduser, $idphoto) {

		global $database_conexion, $conexion;
		mysql_select_db($database_conexion, $conexion);
		$query_NumComent = sprintf ("SELECT * FROM z_images_likes WHERE usuario = %s AND idimagen = %s", 
				GetSQLValueString($iduser, "int"),
				GetSQLValueString($idphoto, "text"));
		$NumComent = mysql_query($query_NumComent, $conexion) or die(mysql_error());
		$row_NumComent = mysql_fetch_assoc($NumComent);
		$totalRows_NumComent = mysql_num_rows($NumComent);
...
function tiempo_transcurrido($fecha) {
	...
	$intervalos = array("second", "minute", "hour", "day", "week", "mounth", "year");
	...
	return "$diferencia $intervalos[$j] $tiempo";
